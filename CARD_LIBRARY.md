...
| 类名 | 游戏内名称 | 类型 | 稀有度 | 费用 | 目标 | 关键字/标签 | 效果摘要 | 备注 |
|---|---|---|---|---:|---|---|---|---|
| AngelsBlessing | 天使祝福 | 能力 | 罕见 | 1 | 自身 | 打牌次数引擎 | 每打出 5/4 张牌，抽 1 张牌；计数跨回合保留，触发后重置 | 去掉入场抽牌和升级抽 2，改为纯连打引擎，避免成为泛用强过牌能力 |
...
| 类名 | 游戏内名称 | 类型 | 稀有度 | 费用 | 目标 | 关键字/标签 | 效果摘要 | 备注 |
|---|---|---|---|---:|---|---|---|---|
| SanctifiedCrossfire | 新约交叉火力 | 攻击 | 先祖 | 1 | 单体敌人 | 速射、易伤、虚弱、生成 Gunspark | 造成 12/16 点伤害，给予 2/3 层易伤和 1 层虚弱，并将 1 张枪火火花加入手牌 | 通过 Archaic Tooth 将起始牌交叉火力替换得到；从纯伤害速射升级为伤害、攻防标记和火花三轴入口，但下调直伤避免速射和火花链路让单费爆发过高 |
| PenguinLogisticsParcel | 企鹅物流快递箱 | 技能 | 先祖 | 1 | 自身 | 消耗、跨职业临时生成、稀有牌 | 随机生成 3 张任意职业的稀有牌加入手牌，它们本回合费用变为 0；升级后从 5 张随机稀有牌中选择 3 张 | 通过 Darv / Dusty Tome 获得；独立于 Archaic Tooth 的 Crossfire 转化，偏娱乐性和跨职业开箱体验 |
...
- 2026-06-13 先祖牌收敛：`SanctifiedCrossfire` 从 16/24 下调为 12/16 伤害，保留 2/3 易伤、1 虚弱和 1 火花，避免速射和生成火花共同放大后单费爆发过高。
...
- 2026-06-13 Darv / Dusty Tome 语义修正：新增 `PenguinLogisticsParcel` 作为独立先祖事件奖励；`SanctifiedCrossfire` 保持只作为 Archaic Tooth 对 `Crossfire` 的先祖替换。
...
- 卡牌类与卡面资源状态：58 个卡牌类含 `Gunspark` token 与 2 张先祖牌；`PenguinLogisticsParcel` 已补正式卡面，主题为能天使打开企鹅物流快递箱。2026-06-05 第二批至第六批新增卡曾使用独立抽象枪火/准星过渡卡面，2026-06-06 已将这些过渡卡面重绘为正式角色卡面。`SanctifiedCrossfire` 与 `PenguinLogisticsParcel` 使用竖向裁切以适配先祖卡框，其余重绘卡面统一为 1024x768；后续实机阶段重点看裁切、亮度和主题辨识度
