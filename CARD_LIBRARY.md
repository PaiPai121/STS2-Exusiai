...
| FullAuto | 全自动 | 攻击 | 稀有 | 2 | 单体敌人 | 速射、消耗 | 造成 5/6 点伤害 3 次 | 从爆发过牌改为多段速射攻击，避免复制牌继续抽牌导致爆发过牌回潮；关键词由关键词栏显示 |
...
| 类名 | 游戏内名称 | 类型 | 稀有度 | 费用 | 目标 | 关键字/标签 | 效果摘要 | 备注 |
|---|---|---|---|---:|---|---|---|---|
| Gunspark | 枪火火花 | 攻击 | 隐藏衍生 | 0 | 单体敌人 | 虚无、消耗 | 造成 5 点伤害，升级 +2 伤害 | 由战术侧闪、快速换弹、追猎指令、枪林弹雨、掩护换弹、突破向量、光环掩护、火控校准、天使装填、终端齐射、华法琳特调生成；生成流程先进入 CombatState，再加入手牌；关键词由关键词栏显示 |
