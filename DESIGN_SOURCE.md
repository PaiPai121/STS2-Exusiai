...
- The current Ancient replacement for `Crossfire` is `SanctifiedCrossfire`: 1 cost, Rapid Fire, 16/24 damage, apply 2/3 Vulnerable and 1 Weak, and add 1 Gunspark. It is intentionally calibrated against Ironclad `Bash` becoming `Break` through Archaic Tooth, so it should feel like a boss-reward transformation rather than a small damage upgrade over the starter card. Weak stays fixed at 1 and does not upgrade, because Rapid Fire can duplicate the application.
