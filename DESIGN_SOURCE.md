...
- The 2026-06-05 convergence balance pass is canonical: `CrossfirePattern` should sit above starter Strike as a common multi-hit payoff, `PointBlankShot` should be a desirable 0-cost Gunspark entry, `FieldStrip` should add its Gunspark directly to hand instead of polluting the draw or discard pile, and `RapidStance` should reward prior Rapid Fire with extra draw.
...
- The current Ancient replacement for `Crossfire` is `SanctifiedCrossfire`: 1 cost, Rapid Fire, 12/16 damage, apply 2/3 Vulnerable and 1 Weak, and add 1 Gunspark. It is intentionally calibrated as a boss-reward transformation, but its direct damage is kept below the earlier 16/24 tuning because Rapid Fire plus the generated Gunspark made the single-card burst too high. Weak stays fixed at 1 and does not upgrade, because Rapid Fire can duplicate the application.
- Darv / Dusty Tome should grant `PenguinLogisticsParcel`, a separate Exusiai Ancient skill themed as a Penguin Logistics delivery box. Base: 1 cost, Exhaust, generate 3 random Rare cards from any available character pool into your hand; they cost 0 this turn. Upgrade: reveal 5 random Rare cards and choose 3. Generated cards are temporary combat cards, but do not gain Ethereal or Exhaust; if not played this turn, they remain for the combat at their normal cost. Do not route this event to `SanctifiedCrossfire`; that card is reserved for Archaic Tooth's `Crossfire` transformation.
- `GunslingerRush` is the defensive Rapid Fire common, not a strict upgrade over starter `Crossfire`: current tuning is 1 cost, Rapid Fire, 5/8 damage and 5/7 Block. Keep `Crossfire` as the higher-damage pure starter unless it is explicitly redesigned later.
- `DeliveryGuaranteed` should preserve healthy infinites through deck compression, but not provide easy precision self-loops in large decks. Current tuning is 2 cost, Exhaust: randomly reveal 5/6 cards from the draw pile, choose up to 2/3 of them, and copy those choices into hand with 0 cost this turn, Ethereal, and Exhaust.
